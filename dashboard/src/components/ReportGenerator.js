...
function getHyperparameterCount(run) {
  return run?.params &&
...
export function generateHTMLReport(
  ...
  const metrics =
    getMetricEntries(run);

  const hyperparameterCount =
...
<div class="metric-card"><div class="metric-value">${hyperparameterCount}</div><div class="metric-label">Hyperparameters</div></div>
</div>
</div>
<div class="section">
<h2>Hyperparameters (${hyperparameterCount})</h2>
${hyperparameterCount ? `<div class="params-grid">${Object.entries(params).map(([key,value]) => `<div class="param-item"><span class="param-key">${escapeHTML(key)}</span><span class="param-value">${escapeHTML(value)}</span></div>`).join('')}</div>` : '<div class="empty-state">No hyperparameters were recorded for this run.</div>'}
</div>
<div class="report-footer">
<span>Generated by <span class="footer-brand">SkiTrack</span></span>
...
export function downloadHTMLReport(
  ...
  try {
    ...
    link.href = url;
    link.download =
      `experiment-report-${run.run_id}.html`;
...
export function generateSummaryReport(
  ...
  const hyperparameterCounts =
    safeRuns.map(
      getHyperparameterCount
    );

  const avgHyperparameters =
    hyperparameterCounts.length
      ? hyperparameterCounts.reduce(
          (sum, value) =>
            sum + value,
          0
        ) /
...
<div class="stat-card"><div class="stat-value">${avgHyperparameters === null ? 'N/A' : avgHyperparameters.toFixed(1)}</div><div class="stat-label">Avg Hyperparameters</div></div>
</div>
<h2>All Experiments</h2>
${safeRuns.length ? `<table><thead><tr><th>#</th><th>Run ID</th><th>Model</th><th>Training Time</th><th>Samples</th><th>Hyperparameters</th></tr></thead><tbody>${safeRuns.map((run,i) => `<tr><td>${i + 1}</td><td><code>${escapeHTML(run?.run_id || 'Not available')}</code></td><td>${escapeHTML(run?.model_name || 'Not available')}</td><td>${escapeHTML(formatTrainingTime(run?.training_time))}</td><td>${escapeHTML(run?.dataset_shape?.[0] ?? 'N/A')}</td><td>${getHyperparameterCount(run)}</td></tr>`).join('')}</tbody></table>` : '<div class="empty-state">No experiment runs are available.</div>'}
<div class="footer">Generated by SkiTrack</div>
</div>
</body>
</html>`;
...
export function downloadSummaryReport(
  ...
  try {
    ...
    link.href = url;
    link.download =
      'experiment-summary-report.html';
